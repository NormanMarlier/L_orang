...
class TestRRRSolver:
    ...
    def test_init_solver_1(self):
        ...
        with pytest.raises(ValueError):
            self.solver = RRRSolver(0, 0, 0, [0, math.pi], [0, math.pi], [0, math.pi])

    def test_init_solver_2(self):
        # If there is no negative length
        with pytest.raises(ValueError):
            self.solver = RRRSolver(-1, 0, 0, [0, math.pi], [0, math.pi], [0, math.pi])

    def test_check_angle_1(self):
        self.solver = RRRSolver(1, 1, 1, [-0.1, 3.14], [-0.1, 3.14], [-0.1, 3.14])
